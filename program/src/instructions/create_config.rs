use solana_cpi::{invoke_signed, invoke};
...
use spl_token_2022::{extension::{default_account_state::DefaultAccountState, BaseStateWithExtensions, PodStateWithExtensions}, instruction::AuthorityType, pod::PodMint};
...
impl<'a> CreateConfig<'a> {
    ...
    pub fn process(&self, remaining_data: &[u8]) -> ProgramResult {
        ...
        let gating_program = Pubkey::try_from(remaining_data).map_err(|_| ProgramError::InvalidInstructionData)?;
...
impl<'a> TryFrom<&'a [AccountInfo<'a>]> for CreateConfig<'a> {
    ...
    fn try_from(accounts: &'a [AccountInfo<'a>]) -> Result<Self, Self::Error> {
        ...
        let (_, config_bump) = Pubkey::find_program_address(&[MintConfig::SEED_PREFIX, mint.key.as_ref()], &crate::ID);
